  ...
  return (
    <div className="perspective h-64 w-full">
      <motion.button
        ...
        aria-label={`Flashcard: ${card.term}. Click to ${flipped ? 'hide' : 'reveal'} the definition.`}
        animate={{ rotateY: flipped ? 180 : 0 }}
        transition={{ duration: 0.6, ease: [0.4, 0, 0.2, 1] }}
        className="preserve-3d relative h-full w-full cursor-pointer rounded-3xl text-left"
      >
        {/* Front */}
        <div className="backface-hidden glass liquid-sheen absolute inset-0 flex flex-col justify-between rounded-3xl p-6">
          ...
          <span className="flex items-center gap-1.5 text-xs font-semibold text-bronze-600">
            <RotateCw className="h-3.5 w-3.5" /> Tap to reveal
          </span>
        </div>

        {/* Back */}
        <div className="backface-hidden rotate-y-180 glass-dark absolute inset-0 flex flex-col justify-between rounded-3xl p-6">
